    ...
    <nav className={` bg-background/95 backdrop-blur-sm transition-shadow ${scrolled ? "shadow-md border-b border-border" : ""}`}>
      <div className="flex items-center justify-between h-16 md:h-20 px-4 sm:px-6">
        <Link to="/" className="flex-shrink-0">
          <img src={grandeurLogo} alt="Grandeur SSCBS" className="h-10 md:h-12 w-auto" />
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-1">
          ...
          <Link
            to="/contact"
            className="ml-4 px-5 py-2.5 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity"
          >
            Get In Touch
          </Link>
        </div>
